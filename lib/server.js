...
*/
function exclude(exclude$, field, obj) {
    ...
    if(options.exclude) {
      if(options.exclude instanceof Array) {
        for(var i=0,len=options.exclude.length;i<len;i++) {
          if(exclude(options.exclude[i], field, obj)) return
        }
      } else {
        if(exclude(options.exclude, field, obj)) return
      }
    }
          ...
          if(stats.isDirectory()) {
            exports.readdir(path, function(err, files) {
              if(err) {
                ...
                if(method && method(path, field, files, obj)) {
                  if(--obj.rdy===0) cb(errs,obj.val)
                  return;
                }
                current[field] = {}
            ...
            current[field] = true
            ;(method && method(path, field, false, obj))
            if(--obj.rdy===0) cb(errs,obj.val)
          }
        }
      })
